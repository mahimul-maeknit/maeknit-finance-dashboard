      ...
      if (status === "authenticated" && session?.user?.email === "mahimul@maeknit.io") {
        try {
          ...
          if (Object.keys(fetchedSettings).length > 0) {
            ...
            setShifts(fetchedSettings.shifts)
            setAvgGarmentPrice(fetchedSettings.avgGarmentPrice)
            setDevelopmentMix(fetchedSettings.developmentMix)
            setSwatchesPerWeek(fetchedSettings.swatchesPerWeek)
            setSamplesPerWeek(fetchedSettings.samplesPerWeek)
            setGradingPerWeek(fetchedSettings.gradingPerWeek)
